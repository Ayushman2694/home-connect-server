...
export default async function verifyOtpHandler(req, res, client) {
    try {
        ...
        const up = await users.findOneAndUpdate(
            { phone: e164 },
            { $setOnInsert: { createdAt: now }, $set: { lastLogin: now, phone: e164, isAddressVerified: false, roles: { resident: true } } },
            { upsert: true, returnDocument: "after" }
        );
        if (!up || !up.value) return res.status(500).json({ error: "User update failed" });
        const user = up.value;
        ...
        return res.json({
            ok: true,
            token,
            user: { id: String(user._id), phone: e164, isAddressVerified: !!user.isAddressVerified, roles: user.roles }
        });
    } catch (error) {
        console.error(error);
